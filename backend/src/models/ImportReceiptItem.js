...
class ImportReceiptItem {
  ...
  static async update(id, itemData, connection = null) {
    ...
    params.push(id);
    const query = `UPDATE import_receipt_items SET ${fields.join(', ')} WHERE id = ?`;
  ...
  static async delete(id, connection = null) {
    const conn = connection || pool;
    const query = 'DELETE FROM import_receipt_items WHERE id = ?';
  ...
  static async deleteByReceiptId(receiptId, connection = null) {
    const conn = connection || pool;
    const query = 'DELETE FROM import_receipt_items WHERE import_receipt_id = ?';
