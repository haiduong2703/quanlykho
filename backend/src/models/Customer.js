...
class Customer {
  ...
  static async findAllActive() {
    const [rows] = await pool.query('SELECT * FROM customers WHERE is_active = TRUE ORDER BY name');
    return rows;
  }
  ...
  static async create(data) {
    const query = `
      INSERT INTO customers (code, name, contact_person, phone, email, address, tax_code, note, is_active)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
  ...
  static async generateCode() {
    const [rows] = await pool.query(
      "SELECT code FROM customers WHERE code LIKE 'KH%' ORDER BY id DESC LIMIT 1"
    );
